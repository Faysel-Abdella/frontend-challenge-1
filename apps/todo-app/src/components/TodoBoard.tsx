...
export default function TodoBoard() {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const destinationTodos = todos.filter(
      todo => todo.status === destination.droppableId && todo.id !== draggableId
    );
    ...
    if (destinationStatusTodos.length === 0) {
      ...
      const insertIndex = updatedTodos.findIndex(
        todo => todo.status === destination.droppableId
      ) + destination.index;
    ...
    toast({
      title: "Task moved",
      description: `"${todoToMove.title}" moved to ${destination.droppableId.replace('-', ' ')}`
    });
  };
